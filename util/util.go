...
var (
	CommandTimeoutErr = errors.New("Command timed out.")
...
func WaitTimeout(c *exec.Cmd, timeout time.Duration) error {
	...
	case <-timer.C:
		...
		<-done
		return CommandTimeoutErr
	}
}

func RandomSleep(base time.Duration, precisionLose float64, shutdown <-chan struct{}) bool {
	if base == 0 || precisionLose < 0.0 || precisionLose > 1.0 {
		return false
	}

	maxSleepDelta := big.NewInt(int64(float64(base.Nanoseconds()) * 2.0 * precisionLose))

	var sleepns int64
	if j, err := rand.Int(rand.Reader, maxSleepDelta); err == nil {
		sleepns = j.Int64() + base.Nanoseconds() - int64(float64(base.Nanoseconds())*precisionLose)
	}
